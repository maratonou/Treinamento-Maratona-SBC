            ...
            for (int k = i; k < n; k++)
            {
                for (int l = j; l < m; l++)
                {
                    if (k-i+1 >= 5 && l-j+1 >= 4)
                    {
                        ...
                        for (int x = i+1; x < k; x++)
                        {
                            moves += prefixes[x][l] - prefixes[x][j+1];
                            // cout << x << " intervalo: [" << l << ", " << j+1 << "] || calculo na linha: " <<
                            // prefixes[x][l] - prefixes[x][j+1] << "\n";
                        }
                        // cout << "\n";
                        movesInside = moves;
                        for (int x = i+1; x < k; x++)
                        {
                            if (matriz[x][l] == '0') moves+= 1;
                            if (matriz[x][j] == '0') moves+= 1;
                        }
                        for (int x = j+1; x < l; x++)
                        {
                            if (matriz[i][x] == '0') moves+= 1;
                            if (matriz[k][x] == '0') moves+= 1;
                        }
                        
                        if (moves < ans)
                        {
                            ans = moves;
                            // print(prefixes, matriz, i, j, k, l);
                            // cout << "\n" << "movesInside " << movesInside << " | movesBorder " << moves-movesInside << "\n" ;
                            // cout << ans << "\n";
                            // return;
                        }
                    }
